...
*iefolder*1*FolderGlobals*R2_Endline********************************************
*iefolder will not work properly if the line above is edited
   ...
   *Encrypted round sub-folder globals
   ...
   global R2_Endline_dtRaw       "$R2_Endline_encrypt/Raw Identified Data" 
   global R2_Endline_doImp       "$R2_Endline_encrypt/Dofiles Import" 
   global R2_Endline_HFC         "$R2_Endline_encrypt/High Frequency Checks" 

   *DataSets sub-folder globals
   global R2_Endline_dt          "$R2_Endline/DataSets" 
   global R2_Endline_dtInt       "$R2_Endline_dt/Intermediate" 
   global R2_Endline_dtFin       "$R2_Endline_dt/Final" 

   *Dofile sub-folder globals
   global R2_Endline_do          "$R2_Endline/Dofiles" 
   global R2_Endline_doCln       "$R2_Endline_do/Cleaning" 
   global R2_Endline_doCon       "$R2_Endline_do/Construct" 
   global R2_Endline_doAnl       "$R2_Endline_do/Analysis" 

   *Output sub-folder globals
   global R2_Endline_out         "$R2_Endline/Output" 
   global R2_Endline_outRaw      "$R2_Endline_out/Raw" 
   global R2_Endline_outFin      "$R2_Endline_out/Final" 

   *Questionnaire sub-folder globals
   global R2_Endline_prld        "$R2_Endline_quest/PreloadData" 
   global R2_Endline_doc         "$R2_Endline_quest/Questionnaire Documentation" 
...
*iefolder will not work properly if the line above is edited
   ...
   *            R2_Endline should be replicated, including output of 
   *            tablets, graphs, etc.
   ...
   * run to 1. To not run a task, set the local to 0.
   ...
   if (`importDo' == 1) { //Change the local above to run or not to run this file
       do "$R2_Endline_doImp/R2_Endline_import_MasterDofile.do" 
   }

   if (`cleaningDo' == 1) { //Change the local above to run or not to run this file
       do "$R2_Endline_do/R2_Endline_cleaning_MasterDofile.do" 
   }

   if (`constructDo' == 1) { //Change the local above to run or not to run this file
       do "$R2_Endline_do/R2_Endline_construct_MasterDofile.do" 
   }

   if (`analysisDo' == 1) { //Change the local above to run or not to run this file
       do "$R2_Endline_do/R2_Endline_analysis_MasterDofile.do" 
   }
